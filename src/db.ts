...
import { generateNewPlayerCard } from './libs/card'
...
export const createNewPlayer = (gameId, numberOfCards: number) => {
	const cards = JSON.stringify(
		new Array(numberOfCards).fill(null).map(() => generateNewPlayerCard()),
	)
	return db
		.collection('players')
		.add({
...
export const writeLastNumber = async (gameId: string, lastNumber: number) => {
	...
	data &&
		db
			.collection('games')
			.doc(gameId)
			.update({ extractedNumbers: data.extractedNumbers.concat(lastNumber) })
			.catch(function (error) {
				console.error('Error writing last number: ', error)
			})
}

export const subscribeLastNumber = (
	gameId: string,
	callback: (lastNumber: number) => void,
) => {
	db.collection('games')
		.doc(gameId)
...
export const getExtractedNumbers = (gameId: string) =>
	db
		.collection('games')
		.doc(gameId)
		.get()
...
export const getPlayerCards = (playerId: string) => {
	return db
		.collection('players')
		.doc(playerId)
		.get()
